...
void enemyInit(UBYTE v){
    ...
    for(UBYTE i=0; i<ENEMY_BULLET_MAX; i++){
        ...
        enemyBullet[i].blockId[0] = 11;
        enemyBullet[i].blockId[1] = 12;
        enemyBullet[i].ani = 11;
    ...
    for(UBYTE i=0; i<ENEMY_BULLET_MAX; i++){
        ...
        if(enemyBullet[i].use==1){
            ...
            if( (enemyBullet[i].x>=role.x)&&(enemyBullet[i].x<=(role.x+role.width))&&
                (enemyBullet[i].y>=role.y)&&(enemyBullet[i].y<=(role.y+role.height))){
                    // 關閉所有子彈.
...
void setup(){
    // 設定使用到的精靈.
    set_sprite_data(0, 15, TileLabel);
